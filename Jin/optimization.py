        ...
        for best_prev in best_for_size_prev:
            ...
            for i in candidate_options_1:
                ...
                if not DEBUG_MODE:
                    ...
                    out.printProgress(" %6.2lf%%   ( path %3d/%d  of  size  %2d/%d )  %16.2lf     (%d %d)" % (
                    100 * pr, c_searched_paths, search_paths, s, LIM_SENTENCES, best_score, len(idx_best_for_size1),
                    len(idx_best_for_size2)), end="\r")
                    out.printdebug()
                ...
                if SPEED == 'CAREFUL':
                    ...
                    summ_cand_1_0 = struct.idx_to_summ(source1,
                                                       idx_best_for_size1)  # Option to keep last summary (and change the other side)
                ...
                for j in candidate_options_2:
                    ...
                    score = method.score(stats_source_1, stats_source_2, stats_cand_1,
                                         stats_cand_2)  # *((len(idx_cand_1)+len(idx_cand_2))**0.1)

                    if SPEED == 'CAREFUL':
                        score_01 = method.score(stats_source_1, stats_source_2, stats_cand_1_0,
                                                stats_cand_2)  # *((len(idx_cand_1)+len(idx_cand_2))**0.1)
                        score_10 = method.score(stats_source_1, stats_source_2, stats_cand_1,
                                                stats_cand_2_0)  # *((len(idx_cand_1)+len(idx_cand_2))**0.1)
