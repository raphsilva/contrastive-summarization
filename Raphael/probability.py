...
def normalDistribution(mean, std, prob, step=DISTRIBUTIONS_STEP, leftlimit=-DISTRIBUTIONS_LENGTH/2, rightlimit=DISTRIBUTIONS_LENGTH/2):
    ...
    for i in [j * step for j in range(left, right)]:
        r[i] = normalContinuous.pdf(i) + INFINITESIMAL  # Soma para evitar divisão por zero no KL (ele retornava inf em alguns casos)
...
def normalDistributionZero(step=DISTRIBUTIONS_STEP, leftlimit=-DISTRIBUTIONS_LENGTH/2, rightlimit=DISTRIBUTIONS_LENGTH/2):
